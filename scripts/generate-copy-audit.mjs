...
const buildIdentityMarker = "<!-- __BUILD_IDENTITY__ -->";
...
function htmlSentences() {
  const html = readFileSync(resolve(root, "site/index.html"), "utf8")
    .replace(buildIdentityMarker, `v${packageVersion} · BUILD_ID`);
...
are excluded. \`BUILD_ID\` represents the one-token commit or factory build ID
injected into each built footer. Run \`npm run copy:audit:check\` to reject stale tables, incorrect
